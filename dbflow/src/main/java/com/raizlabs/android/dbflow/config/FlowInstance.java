...
public class FlowInstance {
    ...
    private static class GlobalDatabaseHolder extends DatabaseHolder {
        ...
        public void setId(@NonNull String id) {
            if (databaseDefinitionMap.isEmpty()) {
                return;
            }

            Map.Entry<Class<?>, DatabaseDefinition> firstEntry = databaseDefinitionMap.entrySet().iterator().next();
            firstEntry.getValue().setId(id);
        }
    ...
    private GlobalDatabaseHolder globalDatabaseHolder = new GlobalDatabaseHolder();

    private HashSet<Class<? extends DatabaseHolder>> loadedModules = new HashSet<>();

    private final String DEFAULT_DATABASE_HOLDER_NAME = "GeneratedDatabaseHolder";

    private final String DEFAULT_DATABASE_HOLDER_PACKAGE_NAME =
            FlowInstance.class.getPackage().getName();

    private final String DEFAULT_DATABASE_HOLDER_CLASSNAME =
    ...
    public FlowInstance(@NonNull  String id) {
        this.id = id;
    }
    ...
    @NonNull
    public String getTableName(Class<?> table) {
     ...
     */
    public Class<?> getTableClassForName(String databaseName, String tableName) {
     ...
     * @return The associated table class for the specified name.
     */
    public Class<?> getTableClassForName(Class<?> databaseClass, String tableName) {
    ...
    @NonNull
    public String getDatabaseName(Class<?> database) {
        return getDatabase(database).getDatabaseName();
    }

    @NonNull
    public DatabaseWrapper getWritableDatabaseForTable(Class<?> table) {
    ...
    @NonNull
    public DatabaseWrapper getWritableDatabase(String databaseName) {
        return getDatabase(databaseName).getWritableDatabase();
    }

    @NonNull
    public DatabaseWrapper getWritableDatabase(Class<?> databaseClass) {
     ...
     */
    public void initModule(Class<? extends DatabaseHolder> generatedClassName) {
     ...
     */
    protected void loadDatabaseHolder(Class<? extends DatabaseHolder> holderClass) {
        if (loadedModules.contains(holderClass)) {
            return;
        }

        try {
            //globalDatabaseHolder.setId(id);
            Constructor<? extends DatabaseHolder> constructor =
                    holderClass.getConstructor(String.class);
            // Load the database holder, and add it to the global collection.
            DatabaseHolder dbHolder = constructor.newInstance(id);
     ...
     */
    public void init(@NonNull FlowConfig flowConfig) {
     ...
     */
    public TypeConverter getTypeConverterForClass(Class<?> objectClass) {
        checkDatabaseHolder();
    ...
    @SuppressWarnings("unchecked")
    @NonNull
    public <TModel> InstanceAdapter<TModel> getInstanceAdapter(Class<TModel> modelClass) {
    ...
    @SuppressWarnings("unchecked")
    @NonNull
    public <TModel> RetrievalAdapter<TModel> getRetrievalAdapter(Class<TModel> modelClass) {
    ...
    @SuppressWarnings("unchecked")
    @NonNull
    public <TModel> ModelAdapter<TModel> getModelAdapter(Class<TModel> modelClass) {
    ...
    @SuppressWarnings("unchecked")
    @NonNull
    public <TModelView> ModelViewAdapter<TModelView> getModelViewAdapter(
    ...
    @SuppressWarnings("unchecked")
    @NonNull
    public <TQueryModel> QueryModelAdapter<TQueryModel> getQueryModelAdapter(
    ...
    @NonNull
    public ModelNotifier getModelNotifierForTable(Class<?> table) {
        return getDatabaseForTable(table).getModelNotifier();
    }

    @NonNull
    public TableNotifierRegister newRegisterForTable(Class<?> table) {
        return getModelNotifierForTable(table).newRegister();
    }

    @Nullable
    private <T> ModelAdapter<T> getModelAdapterOrNull(Class<T> modelClass) {
        return getDatabaseForTable(modelClass).getModelAdapterForTable(modelClass);
    }

    @Nullable
    private <T> ModelViewAdapter<T> getModelViewAdapterOrNull(Class<T> modelClass) {
        return getDatabaseForTable(modelClass).getModelViewAdapterForTable(modelClass);
    }

    @Nullable
    private <T> QueryModelAdapter<T> getQueryModelAdapterOrNull(Class<T> modelClass) {
     ...
     */
    Map<Integer, List<Migration>> getMigrations(String databaseName) {
